...
public abstract class DNSStateTask extends DNSTask {
    ...
    public void run() {
        ...
        try {
            ...
            if (!this.getTaskState().isClosing() && !this.getTaskState().isClosed() && !this.getTaskState().isCanceling() && !this.getTaskState().isCanceled()) {
                logger.warn("{}.run() - trying to recover", this.getName());
                this.recoverTask(e);
            }
            else {
                logger.warn("{}.run() - already closing/closed/cancelling/cancelled, not recovering", this.getName());
            }
        }
        this.advanceTask();
    }
